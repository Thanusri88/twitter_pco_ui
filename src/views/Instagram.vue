                ...
                <div class="form-group">
                  <div>
                    <select
                      ...
                      multiple
                    >
                      <option :value="i.igId" v-for="i in infs" :key="i._id">
                        {{ i.name }}
                      </option>
                    </select>
                  </div>
...
export default {
  ...
  data() {
    return {
      ...
      password: "",
      data: [],
      csvHeader: "data:text/csv;charset=utf-8,Channel,Video URL,Created At\n",
      infs: [],
    };
  },
  created() {
  ...
  methods: {
    ...
    goToNewSheet() {
      ...
      this.data.forEach((record) => {
        ...
        },https://www.youtube.com/watch?v=${record.id.videoId},"${new Date(
        ...
        .then((res) => {
          if (res.data) {
            if (res.data && res.data.payload && res.data.payload) {
              this.infs = res.data.payload.filter((i) => i.igId);
            }
          }
        })
        .catch((error) => {
